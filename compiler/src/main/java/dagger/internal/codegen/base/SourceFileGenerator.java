...
public abstract class SourceFileGenerator<T> {
  ...
  public void generate(T input) throws SourceFileGenerationException {
    for (TopLevelType type : topLevelTypes(input)) {
      try {
  ...
  private JavaFile buildJavaFile(T input, TopLevelType typeSpecBuilder) {
    XElement originatingElement = originatingElement(input);
    typeSpecBuilder.getTypeSpec().addOriginatingElement(toJavac(originatingElement));
    AnnotationSpec generatedAnnotation =
        AnnotationSpec.builder(Constants.GENERATED)
            ...
            .build();
    typeSpecBuilder.getTypeSpec().addAnnotation(generatedAnnotation);

    // TODO(b/134590785): remove this and only suppress annotations locally, if necessary
    typeSpecBuilder.getTypeSpec().addAnnotation(
   ...
   */
  public abstract ImmutableList<TopLevelType> topLevelTypes(T input);
